...
if __name__ == "__main__":
    ...
    for test_case in ["5x5", "11x11", "20x20"]:
        ...
        for algorithm in ["pysat", "backtracking", "bruteforce"]:
            ...
            if algorithm == "bruteforce" and model is not None and hashModel(model) is not None and hashModel(model) >= BRUTEFORCE_LIMIT:
                print("Bruteforce algorithm is too slow for this model. Skipping...\n")
                result += [[test_case, logging_info['CNFs'], logging_info['empties'], len([x for x in model if x > 0]), algorithm, "N/A", hashModel(model)]]

            else:
                print(f"Running {algorithm} on test-case {test_case}...")
                test, algorithm, elapsed_time, logging_info, model = implement(["", algorithm, test_case], False)
