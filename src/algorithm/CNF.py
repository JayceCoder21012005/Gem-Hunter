from Utils.helper import paddingMatrix, getNeighbors, to_1D, subsets_of
...
def generateExactTrapsClauses(cells, k):
    ...
    if k < len(cells):
        ...
        for subset in subsets:
            at_most_clauses.append([-x for x in subset]) # Phủ định để biểu diễn "không phải bẫy".
    ...
    for i in range(1, n_rows - 1):
        for j in range(1, n_cols - 1):
            ...
            if isinstance(cell_value, int):
                ...
                unknown_ids = [to_1D((position[0] - 1, position[1] - 1), original_cols) for position in unknown_cells]

                if unknown_ids:
                    cnf_clauses = generateExactTrapsClauses(unknown_ids, remaining_traps)
