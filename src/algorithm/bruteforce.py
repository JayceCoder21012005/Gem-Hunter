def isValid(KB, assignment, length):
    ...
    for clause in KB:
        ...
        for literal in clause:
            ...
            is_satisfied = (literal > 0 and assignment[var]) or (literal < 0 and not assignment[var])
        ...
        if index == length:
            if min_traps <= trap_count <= max_traps:
                if isValid(KB, current_assignment, length):
                    return [empties_list[i] if current_assignment[empties_list[i]]
                           else -empties_list[i] for i in range(length)]
            return None
