...
from Utils.filehandle import inputMatrix, outputMatrix, printTwoMatrixes, outputCNFs
...
def implement(argv, print_matrix = True):
    ...
    if testcase is None or algorithm is None: return
    ...
    print(f"Test {testcase.lower()}: {logging_info['CNFs']} CNFs - {logging_info['empties']} empty cells")
    if model is None: print("No solution found!.")
    else: print(f"Result hash: #{hashModel(model)} - {len([x for x in model if x > 0])} traps.")
